...
但是，到底应该怎么学习数据分析呢，我觉得有一个很好的入门办法，从现在开始，给自己做的产品梳理一个这样的大网，从核心目标开始层层拆解，放在自己工作台最显眼的位置，随时打开进行分析，养成习惯，持续坚持下去，能形成一个非常清晰的分析框架，无论出现什么问题都能快速找到分析思路。![image-20230211225348377](../../images/2023年2月收藏/image-20230211225348377.png)
...
https://www.aliyundrive.com/s/4gCxwRchace（掌控安全首创安全渗透工程师微专业课）
https://www.aliyundrive.com/s/3yGBD7hVGcW（体系课-Java全栈工程）
https://www.aliyundrive.com/s/Wpfd69FubPt（JavaScript 逆向系列课 + 高阶课）
...
### 笔记
{{< admonition info >}}
优秀的产研团队，一周可以验证十几个想法是否有用、有效、有持续投入可能。
不太行的团队，一周可能只能就一个明显不行的想法开 5 个会，然后出 3 个文档，然后用 2 个月验证这是不行的。
你在第一种团队，用不了 1 万小时，半年你就能参与业内标准制定。
而在第二种团队，你一年过得像是一天重复了200次。
{{< /admonition >}}
{{< admonition tip >}}
**Windows上实用工具推荐**

Wisereminder: 免费的事件提醒工具，可以用作每日的工作提醒。
Motrix: 一款全能的下载工具。支持下载 HTTP 、FTP 、BT 、磁力链接等资源。
洛雪音乐: 一个聚合音乐软件。
Prince: Convert HTML to PDF with CSS，将HTML转换为PDF。
everything： 全局搜索，
ditto：粘贴板工具
snipaste：好用的截图贴图工具
dism++：清爽的清理垃圾管理启动项等
rolan：快捷启动器
utool：快速启动，启动软件，文件，文件夹，系统设置。支持插件安装。
trafficMonitor：设备流量,cpu,内存的监控，直接放在任务栏右侧
quicklook：一键快速打开一些文本文件
Total Commander：是一款应用于Windows 平台的文件管理器，它包含两个并排的窗口，相对 Windows 资源管理器而言多出一些功能。
